...
public final class TransformStage {
  public static void main(String[] args) {
    ...
    final Map<String, Object> producerBaseConfig = Map.of(
    ...
    try (var consumer = new KafkaConsumer<String, Integer>(consumerConfig)) {
      ...
      while (true) {
        ...
        for (var inRec : inRecs) {
          ...
          out.format("Got record with key %s, value %d%n", inKey, inValue);

          // prepare the output record
          final var outValue = inValue * inValue;
          final var outRec = new ProducerRecord<>(outputTopic, inKey, outValue);
          final var topicPartition = new TopicPartition(inRec.topic(), inRec.partition());
          ...
          try {
            ...
            final var nextOffset = new OffsetAndMetadata(inRec.offset() + 1);
