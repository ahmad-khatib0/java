...
public class ProductCompositeIntegration implements ProductService, RecommendationService, ReviewService {
  ...
  @Override
  // the name of the configuration entry to be used for this method
  @Retry(name = "product")
  // The circuit breaker is triggered by an exception, not by a timeout
  // itself. To be able to trigger the circuit breaker after a timeout, we
  // will add a time limiter that can be applied with the annotation @TimeLimiter
  @TimeLimiter(name = "product")
  // The name of the circuit breaker and the time limiter annotation, "product",
  // is used to identify the configuration that will be applied. The
  // fallbackMethod parameter in the circuit breaker annotation is used to specify
  // what fallback method to call ( getProductFallbackValue, in this case) when
  ...
  // apply fallback logic when the circuit breaker
  // is open, that is, when a request fails fast,
  private Mono<Product> getProductFallbackValue(
    ...
    return Mono
        .just(new Product(productId, "Fallback product" + productId, productId, serviceUtil.getServiceAddress()));
  }
