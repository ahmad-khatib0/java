...
function testCompositeCreated() {
  # Expect that the Product Composite for productId $PROD_ID_REVS_RECS has
  # been created with three recommendations and three reviews
...
function testCircuitBreaker() {
  ...
  # endpoints of the product-composite microservice, which are not exposed through the edge server.
  # Therefore, we will access the actuator endpoints by running a command in the product-composite
  # microservice using the Docker Compose exec command
  ...
  # The -T argument is used to disable the use of a terminal for the exec command. This is
  # important to make it possible to run the test-em-all.bash test script in an environment
  # where no terminals exist, for example, in an automated build pipeline used for CI/CD.
  ...
  for ((n = 0; n < 3; n++)); do
    assertCurl 200 "curl -k https://$HOST:$PORT/product-composite/$PROD_ID_REVS_RECS $AUTH -s"
    # verifies that it got a response with data from the underlying database. It does that
    # by comparing the returned product name with the value stored in the database. For the
    # product with product ID 1, the name is "product name C"
  ...
  # The test wraps up by using the /actuator/circuitbreakerevents actuator API, which is exposed
  # by the circuit breaker to reveal internal events. It is used to find out what state
  # transitions the circuit breaker has performed.
