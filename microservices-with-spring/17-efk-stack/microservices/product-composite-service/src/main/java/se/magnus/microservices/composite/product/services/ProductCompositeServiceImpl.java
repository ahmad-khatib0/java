...
public class ProductCompositeServiceImpl implements ProductCompositeService {
  ...
  @Autowired
  public ProductCompositeServiceImpl(ServiceUtil serviceUtil, ObservationUtil observationUtil,
  ...
  public Mono<ProductAggregate> getProduct(HttpHeaders requestHeaders, int productId, int delay, int faultPercent) {
    return observationWithProductInfo(productId,
        () -> getProductInternal(requestHeaders, productId, delay, faultPercent));
  }

  private Mono<ProductAggregate> getProductInternal(HttpHeaders requestHeaders, int productId, int delay,
      int faultPercent) {
    ...
    return Mono.zip(
        values -> createProductAggregate(
            (SecurityContext) values[0], (Product) values[1], (List<Recommendation>) values[2],
  ...
  private ProductAggregate createProductAggregate(
      SecurityContext sc, Product product, List<Recommendation> recommendations, List<Review> reviews,
