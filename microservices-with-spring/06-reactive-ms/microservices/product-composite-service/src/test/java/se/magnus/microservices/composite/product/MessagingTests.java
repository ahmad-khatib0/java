...
// to run tests that verify that the expected messages are sent
class MessagingTests {
  ...
  // can be used to get the messages that were sent during a test.
  @Autowired
  ...
  void createCompositeProduct1() {
    ...
    // an HTTP POST request, requesting the creation of a composite product.
    ...
    // For these tests, the specific timestamp for when an event was created is
    // irrelevant. To be able to compare an actual event with an expected event,
    // ignoring differences in the eventCreatedAt field, a helper class called
    // IsSameEvent can be used. The sameEventExceptCreatedAt() method is a static
    // method in the IsSameEvent class that compares Event objects and treats them
    // as equal if all the fields are equal, except for the eventCreatedAt field.
