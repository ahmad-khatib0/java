...
public class ProductCompositeServiceImpl implements ProductCompositeService {
  ...
  public Mono<ProductAggregate> getProduct(int productId) {
    ...
    // To be able to call the three APIs in parallel, the service implementation
    // uses the static zip() method on the Mono class. The zip method is capable of
    // handling a number of parallel reactive requests and zipping them together
    // once they all are complete
    LOG.info("Will get composite product info for product.id={}", productId);
    return Mono.zip(
        // The first parameter of the zip method is a lambda function that will receive
        // the responses in an array, named values. The array will contain a product, a
        // list of // recommendations, and a list of reviews. The actual aggregation of
        // the responses from the three API calls is handled by the same helper method
        // as before, createProductAggregate(), without any changes.
        ...
        // The parameters after the lambda function are a list of the requests that the
        // zip method will call in parallel, one Mono object per request. In our case,
        // we send in three Mono objects that were created by the methods in the
        // integration class, one for each request that is sent to each core
        // microservice.
